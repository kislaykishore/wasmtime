    ...
    async fn read(
        ...
        match self.table_mut().get_internal_input_stream_mut(&stream)? {
            InternalInputStream::Host(s) => {
                let bytes = HostInputStream::read(s.as_mut(), len as usize)?;
                debug_assert!(bytes.len() <= len as usize);
                Ok(bytes.into())
            }
            InternalInputStream::File(s) => {
                let bytes = FileInputStream::read(s, len as usize).await?;
                debug_assert!(bytes.len() <= len as usize);
                Ok(bytes.into())
            }
        }
    ...
    async fn skip(
        ...
        match self.table_mut().get_internal_input_stream_mut(&stream)? {
            InternalInputStream::Host(s) => {
                ...
                let bytes_skipped = HostInputStream::skip(s.as_mut(), len as usize)?;
                Ok(bytes_skipped as u64)
            }
            InternalInputStream::File(s) => {
                let bytes_skipped = FileInputStream::skip(s, len as usize).await?;
                Ok(bytes_skipped as u64)
            }
        }
    }
